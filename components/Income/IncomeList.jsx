...
import { Edit2, Trash2, Calendar, TrendingUp, Tag, Loader2 } from 'lucide-react';
...
export default function IncomeList({ incomes, onEdit, onDelete }) {
  ...
  const displayedIncomes = filteredIncomes.slice(0, displayedCount);
  const hasMore = displayedCount < filteredIncomes.length;
  ...
  return (
    <div className="space-y-4">
            ...
            >
              <div className="flex-1">
                <div className="flex items-center space-x-3 mb-1">
                  <span className="font-semibold text-gray-900 dark:text-white text-lg">₹{income.amount}</span>
                ...
                <div className="flex items-center space-x-4 text-xs text-gray-600 dark:text-gray-400">
                  ...
                  {income.note && (
                    <div className="flex items-center space-x-1">
                      <Tag size={12} />
                      <span className="truncate max-w-[150px]">{income.note}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          >
            {isFetchingMore ? (
              ...
              >
                ...
                <span className="text-sm font-medium">Loading more incomes...</span>
              </motion.div>
            ) : (
      ...
      <ConfirmDeleteModal 
        ...
        onConfirm={() => onDelete(incomeToDelete)}
        itemName="Income"
      />
    </div>
  );
